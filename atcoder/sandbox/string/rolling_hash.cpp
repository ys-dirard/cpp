...
struct RollingHash{
  ...
  static inline uint64_t generate_base(){
    return ((uint64_t)(rand()) + (uint64_t)(rand()) * (1ull<<31ull)) % (mod-1ull) + 1ull;
  }

  inline void expand(size_t n){
    size_t pn = pow_table.size();
    if(pn < n+1){
  ...
  // [l, l+n)のハッシュ値を返す(lからn個分), 0-origin
...
int main() {
  ...
  string t; cin >> t;
  auto s = t;
  reverse(begin(t) + n, end(t));
  RollingHash rh;
  auto hs = rh.build(t);

  for(int i = 0; i <= n; ++i){
    auto l1 = rh.query(hs, 0, i);
    auto l2 = rh.query(hs, i, n - i);
    auto r1 = rh.query(hs, n, n - i);
    auto r2 = rh.query(hs, n * 2 - i);
