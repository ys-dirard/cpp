...
int main(){
    ...
    Polygon convex_full = andrew_scan(g);

    cout << convex_full.size() << endl;
    int y_min_pos = 0;
    for(int i=0;i<convex_full.size();i++){
        if(convex_full[i].y < convex_full[y_min_pos].y){
            y_min_pos = i;
        }
    }
    for(int i=0;i<convex_full.size();i++){
        cout << convex_full[(i+y_min_pos) % convex_full.size()].x << " " << convex_full[(i+y_min_pos) % convex_full.size()].y << endl;
    }
    return 0;
}